...
│   ├── pusher.py          # Server酱 / 企业微信推送
│   ├── store.py           # SQLite 存状态，做「上次 vs 这次」比对
│   └── fetchers/
│       ├── base.py        # 基类 + 数据模型
│       ├── educoder.py    # 头歌抓取器
│       └── mooc.py        # 中国大学MOOC抓取器
├── refresh_cookies.py     # 自动登录刷新 Cookie
...
如果你不想用自动刷新（愿意手动复制 Cookie），playwright 可以不装。
...
另外配套了几个 `.bat`（`run.bat` / `run-summary.bat` / `refresh.bat`），双击就能跑，方便不熟命令行的用。它们没提交到仓库（.gitignore 里了），需要的话自己建，内容很简单：
